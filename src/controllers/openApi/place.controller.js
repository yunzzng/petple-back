...
const { formatPlaceData, formatPlaceDetail } = require("../../utils/openApi/formatPlaceData");
...
class PlaceController {
  async getTravelData(req, res, next) {
    try {
      ...
      if (!category) {
        throw createError(400, "카테고리를 선택해주세요.");
      }
  ...
  async getTravelDetail(req, res, next) {
    try {
      ...
      if (!id) {
        throw createError(400, "유효한 장소 ID가 필요합니다.");
      }
      ...
      if (!item) {
        throw createError(404, "장소 정보를 찾을 수 없습니다.");
      }
